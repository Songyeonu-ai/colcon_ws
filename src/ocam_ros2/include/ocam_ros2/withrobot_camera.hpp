...
struct camera_control_menu {
  ...
  void dbg_print() { DBG_PRINTF("Index: %d, Name: %s, Value: %d", index, name, value); }
};
...
struct camera_control {
  ...
  void dbg_print() {
    DBG_PRINTF("Id: %u, Name: %s, Value(default [min, step, max]): %d ( %d [%d, %d, %d] ), Type: %d, Flag: %d", id, name.c_str(), value, default_value, minimum, step, maximum, type, flags);
